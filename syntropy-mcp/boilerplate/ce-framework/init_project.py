...
class ProjectInitializer:
    ...
    def __init__(self, target_project: Path, dry_run: bool = False):
        ...
        # Paths to framework packages (bundled in same directory as this script)
        # This script is in syntropy-mcp/boilerplate/ce-framework/
        # Framework packages are co-located in the same directory
        ...
        if not self.infrastructure_xml.exists():
            raise FileNotFoundError(f"Framework package not found: {self.infrastructure_xml}")
        if not self.workflow_xml.exists():
            raise FileNotFoundError(f"Workflow package not found: {self.workflow_xml}")

        # Set ctx_eng_root for blend phase (uses extracted tools after installation)
        # This points to the target project's .ce/tools directory after extraction
        self.ctx_eng_root = None  # Will be set to target_project/.ce after extraction
    ...
    def extract(self) -> Dict:
        ...
        try:
            # Load bundled repomix_unpack module (co-located in same directory)
            ...
            if spec is None or spec.loader is None:
                raise ImportError(f"Failed to load repomix_unpack from {repomix_path}")
    ...
    def blend(self) -> Dict:
        ...
        try:
            ...
            result = subprocess.run(
                ...
                cwd=self.tools_dir,  # Use extracted tools in target project
                capture_output=True,
                text=True,
                timeout=120
            )
