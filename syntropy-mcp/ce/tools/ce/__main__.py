...
from .config_utils import load_config, ConfigError
...
Examples:
  ...
  ce run_py --file tmp/script.py --args "--input data.csv"
    ...
    migrate_config_parser = subparsers.add_parser(
        "migrate-config",
        help="Migrate linear-defaults.yml to config.yml profile section"
    )
    ...
    # Skip validation for setup/migration commands that create/fix config
    skip_validation_commands = [
        "migrate-config",  # Migrates old config to new format
        # "init-project",  # Future: Initialize new project with config template
    ]
    ...
    if args.command == "validate":
        ...
        return cmd_migrate_config(args)
    else:
